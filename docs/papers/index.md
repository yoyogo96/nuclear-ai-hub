...
> 지난 24시간 기준 arXiv 공개 검색 및 arXiv API 보완 조회에서 기존 반영본과 중복되지 않는 원자력+AI 핵심 신규 논문/프리프린트를 확인하지 못했습니다. 검색 범위에는 nuclear, reactor, fusion, plasma, fission과 machine learning/neural/artificial intelligence 조합을 포함했으며, arXiv API는 rate limit 응답이 있어 arXiv 웹 검색으로 교차 확인했습니다.
...
> 지난 24시간 기준 arXiv 공개 검색에서 기존 반영본과 중복되지 않는 원자력+AI 신규 논문은 확인되지 않았습니다. 검색 범위에는 nuclear reactor, fusion, plasma, fission, isotope와 machine learning/neural/artificial intelligence 조합을 포함했습니다.
...
- 오늘은 논문 목록을 추가하지 않고 "신규 없음" 상태로 갱신합니다.
...
## [Challenges and Opportunities for AI to Help Deliver Fusion Energy](https://arxiv.org/abs/2603.25777) (arXiv, 2026-03)
- 저자: 다기관 공동 연구
- 연도/저널: 2026-03, arXiv
- 한 줄 요약: AI가 핵융합 상용화를 가속화할 수 있는 기회와 과제를 종합적으로 분석한 관점 논문. 재료 개발, 엔지니어링, 실시간 플라즈마 제어, 원격 조작 등에서 AI의 역할을 정리하고, 물리 기반 파운데이션 모델과 자동 의사결정의 잠재력을 강조.

### 핵심 기여
1. 핵융합 전 주기에 걸친 AI 적용 기회 체계화 (재료→설계→운전→유지보수)
2. 물리 기반 파운데이션 모델(foundation models for physics) 도입의 필요성 제시
3. 데이터 희소성, 불확실성 정량화, AI-융합 전문가 협력 강화 과제 식별

---

## [Data-Driven Surrogate Models for Nuclear Reactor Core Simulation: Load-Following Applications](https://arxiv.org/abs/2603.14058) (arXiv, 2026-03)
- 저자: 다기관 연구진
- 연도/저널: 2026-03, arXiv
- 한 줄 요약: 원자로 노심 부하추종(load-following) 시뮬레이션을 위한 데이터 기반 서로게이트 모델을 제안. XGBoost를 강성(stiff) 상미분방정식 적분기로 활용해 복잡한 동적 시스템을 고정밀도로 근사하는 혁신적 접근.

### 핵심 기여
1. XGBoost 기반 서로게이트 모델로 강성 ODE 적분 정확도 실증
2. 부하추종 시나리오에서 실시간 노심 시뮬레이션 가속화 가능성 입증
3. 데이터 기반 방법과 물리 기반 모델의 하이브리드 프레임워크 제안
...
- 지난 24시간 기준으로 arXiv를 재확인했으나, 기존 반영본(2026-04-08 3건)과 중복되지 않는 원자력+AI 핵심 신규 논문은 확인되지 않았습니다.
- 오늘은 논문 목록을 추가하지 않고 "신규 없음" 상태로 갱신합니다.
...
- 저자: Xinyang Wang 외
- 연도/학회(저널): 2026-03-31, arXiv (hep-ph / nucl-th)
- 한 줄 요약: 연구진이 물리정보 기반 머신러닝으로 자화된 QCD 물질의 NJL 결합상수와 쿼크 비정상 자기모멘트 함수를 학습해, 강한 자기장 아래 핵물질 상전이와 inverse magnetic catalysis를 더 정밀하게 재현하는 방법을 제시했다.

### 핵심 기여
1. lattice QCD 응축물 데이터를 정답으로 사용해 NJL 유효모형의 자기장 의존 파라미터를 연속 함수 형태로 복원
2. gap equation을 미분가능한 물리 제약 모듈로 포함해, 순수 블랙박스 회귀보다 해석 가능성과 물리 일관성을 높임
3. 강한 자기장에서 결합상수와 AMM이 함께 억제되는 경향을 정량화해 자화 핵물질 미시구조 해석에 새 단서를 제공

## [Data-Driven Optimisation of Superconducting Magnets at CEA Paris-Saclay](https://arxiv.org/abs/2603.29740)
- 저자: Damien F. G. Minenna 외
- 연도/학회(저널): 2026-03-31, arXiv
- 한 줄 요약: CEA Paris-Saclay 연구진이 입자가속기용 초전도 자석 설계에 AI 기반 최적화·데이터 관리 플랫폼을 적용해, 다물리 결합 문제와 복잡한 설계 탐색을 더 빠르게 수행하는 워크플로우를 제안했다.

### 핵심 기여
1. 초전도 자석 설계·해석 데이터를 통합 관리하는 AI 기반 최적화 플랫폼을 구축해 설계 반복 비용을 낮춤
2. 머신러닝과 고급 최적화 기법으로 다변수 자석 형상·성능 탐색을 가속하는 적용 사례를 제시
3. 핵·입자 실험 인프라의 고성능 자석 설계 자동화 가능성을 보여주며 향후 가속기·핵융합 장치 최적화로의 확장 기반을 마련
...
## [A Telescope System for Charge and Position Measurement of High Energy Nuclei](https://arxiv.org/abs/2603.25080)
- 저자: Dexing Miao 외
- 연도/학회(저널): 2026-03-26, arXiv (physics.ins-det)
- 한 줄 요약: 고에너지 중이온 빔 실험용 실리콘 망원경 시스템에 하이브리드 머신러닝 알고리즘을 적용해 핵전하(Z=1~29)와 위치를 동시에 고정밀 측정함으로써, 핵·입자 실험 계측에서 ML 기반 전하 판별 정확도를 크게 끌어올린 연구.

### 핵심 기여
1. 9층 실리콘 마이크로스트립 검출기 기반 대면적·저물질량 망원경 시스템으로 고에너지 핵종 측정 정밀도 향상
2. SSD 전하 측정 난점을 해결하기 위해 하이브리드 머신러닝 알고리즘을 도입해 전하 분해능 0.16 이하 달성
3. 핵전하와 위치를 동시에 정밀 추정해 향후 우주선·중이온·핵계측 실험의 데이터 품질 향상 기반 제시
...
- 저자: Xishuo Wei 외
- 연도/학회(저널): 2026-03-18, arXiv (physics.plasm-ph)
- 한 줄 요약: 최적화된 스텔러레이터의 고차원 형상 공간이 실제로는 저차원 잠재공간에 가깝다는 점을 딥러닝으로 찾아내, 난류 수송을 빠르게 예측하는 대리모델 학습과 차세대 핵융합로 형상 최적화를 더 효율적으로 수행할 수 있음을 보인 연구.

### 핵심 기여
1. 준-헬리컬 대칭(QH) 스텔러레이터 형상이 저차원 잠재공간에 분포함을 딥러닝으로 명시적으로 추출
2. 고비용 전역 gyrokinetic 시뮬레이션을 대체·보조할 ML 대리모델용 학습데이터 생성 효율을 높일 기반 제시
3. 난류 수송과 energetic particle instability, MHD mode를 함께 고려하는 스텔러레이터 형상 최적화 워크플로우로 확장 가능성 제시
